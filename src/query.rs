...
pub fn get_quotas(
    ...
    to_json_binary(&RATE_LIMIT_TRACKERS.load(storage, path.into())?)
}
...
pub fn get_roles(storage: &dyn Storage, owner: String) -> StdResult<Binary> {
    to_json_binary(&RBAC_PERMISSIONS.load(storage, owner)?)
}
...
pub fn get_denom_restrictions(storage: &dyn Storage, denom: String) -> StdResult<Binary> {
    to_json_binary(&ACCEPTED_CHANNELS_FOR_RESTRICTED_DENOM.load(storage, denom)?)
}
...
mod test {
    ...
    fn test_get_role_owners() {
        ...
        let decoded: Vec<String> = from_json(response).unwrap();
    ...
    fn test_get_roles() {
        ...
        let decoded: Vec<Roles> = from_json(response).unwrap();
        assert_eq!(decoded.len(), 1);
